...
data_opts = models.norma_encode()

encoder_list = data_opts[0]
data_scaler = data_opts[1]

stage_fear_encoder = encoder_list[0]
draiand_event_encoder = encoder_list[1]
personality_encoder = encoder_list[2]
...
norma_array = data_scaler.transform(user_dataframe_encoded[features_to_normalize])
...
while bad_input:
    ...
    if user_input == 1:
        bad_input = False
        print("You have choose to use our linear regression model")
        lr_model = models.linear_model()
        lr_model_prdeiction = lr_model.predict(full_norma_dataframe)
        if lr_model_prdeiction < 0.5:
    ...
    # DT
        ...
        bad_input = False
        print("You have choose to use our decision tree model")
